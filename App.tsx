                ...
                {isSearching ? (
                  <div className="absolute inset-0 bg-white md:rounded-sm z-50 flex items-center px-4 md:px-6">
                    ...
                    <input 
                      ...
                      autoFocus
                      className="w-full bg-transparent outline-none font-bold uppercase tracking-wider text-xs text-stone-900 placeholder:text-stone-400"
                      value={searchQuery}
                      onChange={(e) => handleSearchChange(e.target.value)}
                    />
                    <button onClick={() => setIsSearching(false)} className="p-2 text-stone-400 hover:text-stone-900">
                      <X size={16} />
                    </button>
                  </div>
                ) : (
                  ...
                  >
                    ...
                    <div className="truncate">
                      ...
                      <span className={`block text-xs font-bold uppercase tracking-wider truncate ${!selectedCity ? 'text-stone-400' : 'text-stone-900'}`}>
                        {selectedCity ? selectedCity.name : 'Kies een locatie'}
                      </span>
                    </div>
                  </button>
                )}
                ...
                {isSearching && (searchQuery.trim().length >= 1 || searchResults.length > 0) && (
                  <div className="absolute top-full left-0 right-0 bg-white border border-stone-200 md:rounded-sm shadow-2xl z-[60] mt-2 max-h-80 overflow-y-auto animate-fadeIn">
                    <button 
                      ...
                      className="w-full flex items-center gap-3 p-4 hover:bg-emerald-50 text-emerald-800 font-black transition-colors border-b border-stone-100 uppercase text-[9px] tracking-widest"
                    >
                      <Navigation size={12} />
                      Huidige locatie
                    </button>
                    {searchResults.map(city => (
                      <div 
                        ...
                        className="p-4 hover:bg-stone-900 hover:text-white cursor-pointer transition-colors border-b border-stone-50 flex flex-col"
                      >
              ...
              <div ref={monthRef} className="relative flex-1 md:bg-white md:border md:border-stone-200 md:rounded-sm md:shadow-sm transition-all hover:shadow-md h-full">
                ...
                >
                  <div className="truncate">
                    ...
                    <span className={`block text-xs font-bold uppercase tracking-wider truncate ${!selectedMonth ? 'text-stone-400' : 'text-stone-900'}`}>
                      {selectedMonth ? MONTHS[selectedMonth - 1] : 'Selecteer maand'}
                    </span>
                  </div>
                  ...
                  <div className="absolute top-full left-0 right-0 bg-white border border-stone-200 md:rounded-sm shadow-2xl z-[60] mt-2 max-h-80 overflow-y-auto animate-fadeIn">
                    {MONTHS.map((m, i) => (
                      <button
                        ...
                        className={`w-full flex items-center gap-4 p-4 transition-colors border-b border-stone-50 last:border-0 ${
                          selectedMonth === i + 1 ? 'bg-emerald-50 text-emerald-900 font-bold' : 'hover:bg-stone-50 text-stone-600'
                        }`}
                      >
              ...
              >
                <div className="flex items-center gap-3">
                  <div className="text-emerald-600">
                    <CategoryIcon cat={activeCategory} size={16} />
                  </div>
                  <div>
                    <span className="block text-[7px] font-black text-stone-600 uppercase tracking-widest mb-0">Wat zoek je?</span>
                    <span className={`block text-xs font-bold uppercase tracking-wider ${!activeCategory ? 'text-stone-400' : 'text-stone-900'}`}>
                      {activeCategory || 'Maak een keuze...'}
                    </span>
                  </div>
                </div>
                ...
                <div className="absolute top-full left-0 right-0 bg-white border border-stone-200 md:rounded-sm shadow-2xl z-50 mt-2 animate-fadeIn overflow-hidden">
                  {(Object.values(Category) as Category[]).map((cat) => (
                    <button
                      ...
                      className={`w-full flex items-center gap-4 p-5 transition-colors border-b border-stone-50 last:border-0 ${
                        activeCategory === cat ? 'bg-emerald-50 text-emerald-900' : 'hover:bg-stone-50 text-stone-600'
                      }`}
                    >
                      <div className={activeCategory === cat ? 'text-emerald-600' : 'text-stone-400'}>
          ...
          {!selectedCity ? (
            ...
            <>
              <div className="mb-8 flex flex-col sm:flex-row sm:items-end justify-between border-b-2 border-stone-900 pb-8 gap-4">
                <div>
                  <h2 className="text-5xl font-serif font-bold tracking-tighter leading-none mb-3 text-stone-900">
                    {selectedCity.name}
                  </h2>
                  <div className="flex items-center gap-3">
                    <span className={`px-3 py-1 text-[10px] font-black uppercase tracking-widest rounded-full border ${isEndangeredCategory ? 'bg-red-50 text-red-600 border-red-100' : 'bg-stone-100 text-stone-500 border-stone-200'}`}>
                      {activeCategory}
                    </span>

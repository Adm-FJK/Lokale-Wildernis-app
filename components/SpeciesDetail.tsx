      ...
      <div className={`relative bg-white w-full max-w-5xl transition-all duration-500 ease-out transform ${species ? 'scale-100 translate-y-0 opacity-100' : 'scale-95 translate-y-12 opacity-0'} h-full sm:h-auto sm:max-h-[92vh] overflow-hidden shadow-2xl sm:rounded-3xl flex flex-col`}>
        ...
        <div className="flex-1 overflow-y-auto lg:overflow-hidden flex flex-col lg:flex-row bg-white">
          <div className="lg:w-[45%] bg-stone-50 flex shrink-0 border-b lg:border-b-0 lg:border-r border-stone-100 relative">
            <div className="w-full h-full p-4 sm:p-8 flex items-center justify-center min-h-[300px] lg:min-h-0">
              <img 
                src={highResImage || species.imageUrl} 
                alt={species.scientificName} 
                className={`max-w-full max-h-[40vh] lg:max-h-[80vh] w-auto h-auto object-contain drop-shadow-xl transition-all duration-700 ${loading ? 'opacity-20 blur-md' : 'opacity-100'}`}
              />
              {loading && (
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="w-10 h-10 border-4 border-emerald-900 border-t-transparent animate-spin rounded-full" />
                </div>
              )}
            </div>
          </div>
          ...
          <div className="lg:w-[55%] flex flex-col bg-white overflow-visible lg:overflow-hidden">
            <div className="flex-1 lg:overflow-y-auto p-6 md:p-12 lg:p-16">
              <header className="mb-8">
                <div className="h-1 w-12 bg-emerald-600 mb-6 rounded-full"></div>
                
                {species.conservationStatus && (
                  <div className="space-y-3 mb-6">
                    <div className="flex items-center gap-2 bg-red-50 text-red-700 px-3 py-1.5 rounded-lg border border-red-100 w-fit">
                      <ShieldAlert size={14} />
                      <span className="text-[9px] font-black uppercase tracking-widest">{fullStatusText}</span>
                    </div>
                    {iucnDetail && !isEndangeredNL && (
                      <p className="text-[10px] leading-snug text-stone-500 italic max-w-sm">
                        <span className="font-bold">Internationale IUCN-status:</span> {iucnDetail.description}
                      </p>
                    )}
                  </div>
                )}

                <h1 className="text-3xl md:text-4xl lg:text-5xl font-serif font-bold text-stone-900 leading-tight mb-2 tracking-tighter uppercase break-words">
                  {details.dutchName}
                </h1>
                <p className="text-base italic text-stone-400 font-serif break-words">
                  {species.scientificName}
                </p>
              </header>
              
              <div className="prose prose-stone max-w-none mb-10">
                <p className="text-base md:text-xl text-stone-800 leading-relaxed italic font-serif border-l-4 border-stone-100 pl-4 md:pl-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-auto">
                {details.wikiUrl && (
                  <a href={details.wikiUrl} target="_blank" rel="noopener noreferrer" 
                    className="flex items-center justify-center gap-2 py-3.5 bg-white text-stone-900 border-2 border-stone-900 text-[9px] font-black uppercase tracking-widest hover:bg-stone-50 transition-all rounded-xl shadow-md">
                    <BookOpen size={14} /> Bron Wikipedia
                  </a>
                )}
                
                <a href={waarnemingUrl} target="_blank" rel="noopener noreferrer" 
                  className="flex items-center justify-center gap-2 py-3.5 bg-emerald-900 text-white text-[9px] font-black uppercase tracking-widest hover:bg-emerald-800 transition-all rounded-xl shadow-md">
                  <ExternalLink size={14} /> Waarneming.nl
                </a>

                {details.redListLink && (
                  <a href={details.redListLink} target="_blank" rel="noopener noreferrer" 
                    className="sm:col-span-2 flex items-center justify-center gap-2 py-3.5 bg-white text-stone-900 border-2 border-stone-900 text-[9px] font-black uppercase tracking-widest hover:bg-stone-50 transition-all rounded-xl shadow-md">
